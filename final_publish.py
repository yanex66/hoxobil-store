...
def emergency_publish():
    ...
    if not products:
        print("❌ No products found in your shop.")
        return

    for p in products:
        p_id = p['id']
        print(f"🚀 Triggering Publish for: {p['title']}...")
        
        # 2. Trigger the Publish Endpoint to generate 10-digit IDs
        ...
        if res.status_code in [200, 201, 202]:
            print(f"✅ Success! Moving {p['title']} to Published.")
        else:
            print(f"❌ Error for {p['title']}: {res.text}")
